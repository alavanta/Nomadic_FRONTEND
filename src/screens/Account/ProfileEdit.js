...
import { editUser, getUserById } from '../../public/redux/action/user';
...
class ProfileEdit extends Component {
  ...
  userEdit = (name, password, email, phone, address, gender) => {
    this.props.dispatch(
      editUser(name, password, email, phone, address, gender)
    );
    console.warn(
      this.state.name,
      this.state.password,
  ...
  render() {
    console.log(this.props.data[0], 'DARI RENDER');
    return (
      <View
        style={{
          flex: 1,
      ...
      >
        <View style={styles.header}>
          ...
          >
            {this.state.name == '' ? (
              <Icon name="check-circle" type="feather" color="#808080" />
            ) : this.state.email == '' ? (
              <Icon name="check-circle" type="feather" color="#808080" />
            ) : this.state.address == '' ? (
              <Icon name="check-circle" type="feather" color="#808080" />
